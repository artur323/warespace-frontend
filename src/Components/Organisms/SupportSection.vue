<template>
  <div class="relative isolate overflow-hidden bg-gray-900 py-12 sm:py-20 font-poppins">
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="mx-auto max-w-2xl lg:mx-0">
        <h2 class="text-4xl font-bold tracking-tight text-snowwhite sm:text-6xl">Support center</h2>
      ...
      <div class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
        <div v-for="card in cards" :key="card.name" class="flex gap-x-4 rounded-xl bg-snowwhite/5 p-6 ring-1 ring-inset ring-snowwhite/10">
          <component :is="card.icon" class="h-7 w-5 flex-none text-wsblue-400" aria-hidden="true" />
          <div class="text-base leading-7">
            <h3 class="font-semibold text-snowwhite">{{ card.name }}</h3>
