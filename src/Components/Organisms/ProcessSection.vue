          ...
          <div class="lg:max-w-lg">
            ...
            <dl class="mt-10 max-w-xl space-y-8 text-base leading-7 text-gray-600 lg:max-w-none">
              <div v-for="feature in features" :key="feature.name" class="relative pl-9">
                <dt class="inline font-semibold text-gray-900 dark:text-snowwhite">
                  <component :is="feature.icon" class="absolute left-1 top-1 h-5 w-5 text-wsblue-600 dark:text-wsblue-300" aria-hidden="true" />
                  {{ feature.name }}
                </dt>
                {{ ' ' }}
                <dd class="inline dark:text-gray-500">{{ feature.description }}</dd>
              </div>
            </dl>
          </div>
        </div>
